    ...
    try:
        ...
        if output_path.is_file():
            ...
            def replace_main_content(match):
                ...
                closing_tag = match.group(2)  # </main>
                return f"{opening_tag}\n{html}\n{closing_tag}"
            output = re.sub(r'(<main role="main"[^>]*>)[\s\S]*?(</main>)', replace_main_content, output)
        else:
            # If output file does not exist, create it with template
            output = template.replace("{{ article }}", html)
        output = output.replace("{{ title }}", title)
        output = output.replace("{{ filename }}", output_filename.replace("index.html", ""))
...
def process_single_file_with_output(file_path: Path, template: str, output_path: Path, token: Optional[str] = None) -> bool:
    ...
    try:
        ...
        if output_path.is_file():
            ...
            output = template.replace("{{ article }}", html)
        output = output.replace("{{ title }}", title)
        output = output.replace("{{ filename }}", output_path.name.replace("index.html", ""))
        # Write output file
        with open(output_path, 'w', encoding='utf-8') as f:
            f.write(output)
