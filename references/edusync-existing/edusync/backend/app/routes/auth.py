...
async def login(login_data: UserLogin, background_tasks: BackgroundTasks):
    ...
    try:
        # Search by email, username, or IDs
        search_query = {
            "$or": [
                {"email": login_data.email},
                {"username": login_data.email},
                {"roll_number": login_data.email},
                {"faculty_id": login_data.email},
                {"hod_id": login_data.email}
            ]
        }
        user = await users_collection.find_one(search_query)
