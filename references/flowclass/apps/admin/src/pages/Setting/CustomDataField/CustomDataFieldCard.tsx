...
import EditField from './EditCustomDataField'
...
const FieldCard = ({
  ...
  const handleEdit = () => {
    if (data.id) {
      ...
      setCurrentInformationField(data.id)
    }
  }
  return (
    <Box
    ...
    >
      ...
      {data.isDefault ? (
        ...
        <Box css={{ width: '20%' }} justify="flex-end">
          ...
          <SvgIcon
            ...
            data-testid="delete"
            id={`${generateDataTestId('delete-field', data.question)}`}
          >
      ...
      <CustomedAlertDialog
        ...
        description={t('setting:studentInformation.descriptionDelete')}
        title={`${t('setting:studentInformation.titleDeleteDialog')} ${
          data.question
        }`}
      ...
      />
      <EditField open={isOpenEdit} handleClose={() => setIsOpenEdit(false)} />
    </Box>
  )
}
