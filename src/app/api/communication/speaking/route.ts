...
import { evaluateSpeaking } from "@/lib/ai-evaluation-service";
import { checkRateLimit } from "@/lib/rate-limiter";
import { z } from "zod";

const speakingRequestSchema = z.object({
  contentId: z.string().min(1, "Content ID is required"),
  transcribedText: z.string().min(2, "Transcription must be at least 2 characters long"),
});
...
export async function POST(req: Request) {
  try {
    ...
    const rateLimit = checkRateLimit(userId, "speaking", 30000);
    ...
    const parsedData = speakingRequestSchema.parse(body);

    const result = await evaluateSpeaking(userId, parsedData.contentId, parsedData.transcribedText);
