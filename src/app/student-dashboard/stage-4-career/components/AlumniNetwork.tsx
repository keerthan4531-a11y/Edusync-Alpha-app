    ...
    <div className="space-y-6 animate-in fade-in duration-500 relative">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
        {ALUMNI_LIST.map(alumni => {
          ...
          return (
            <LiquidGlassCard key={alumni.id} className="p-6 flex flex-col justify-between" accentColor="#6366f1">
              <div>
                <div className="flex items-center gap-4 mb-4">
                  <div className="w-14 h-14 rounded-full bg-gradient-to-tr from-indigo-500 to-indigo-500 flex items-center justify-center font-bold text-white text-lg shadow-sm">
                    {alumni.avatar}
                  </div>
                  <div>
                    <h3 className="text-[17px] font-semibold text-foreground leading-snug">{alumni.name}</h3>
                    <div className="text-[13px] text-indigo-500 font-semibold">{alumni.role}</div>
              ...
              <button
                ...
                className={`w-full font-semibold py-3 rounded-xl transition-all text-sm flex items-center justify-center gap-2 ${
                  isConnected
                    ...
                    : "bg-indigo-500 hover:bg-indigo-600 text-white shadow-[0_4px_12px_rgba(139,92,246,0.3)] hover:-translate-y-0.5 active:translate-y-0"
                }`}
              >
                {isConnected ? (
                  <>
